...
def load_driver_laps(year: int, gp: str, session_name: str, driver_alias: str):
    ...
    df_laps_driver = laps[['Driver', 'LapTime', 'LapNumber']].reset_index().drop('index', axis=1)
    df_laps_driver['LapTime'] = df_laps_driver['LapTime'].apply(time_delta_to_str)
    df_laps_driver.to_csv(f'../src/data/lapTimeDataTest_{driver_alias}.csv', index=False)

def validate_fast_f1_Request(year: int, gp: str, session_name: str, driver_alias: str):
    if gp == None or year == None or session_name == None or driver_alias == None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Entry 4 parameters to get lap time from a driver in a concrete GP year")
